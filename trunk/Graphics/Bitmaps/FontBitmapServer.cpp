...
void CFbsControl::LoadBitmapL(CFbsBitmap* aBitMap, const TDesC& aPathAndFile,
		TInt aId, TBool aShareIfLoaded)
	{
	TParse mbfn;
...
void CFbsControl::DrawSmiley(CGraphicsContext& aGc)
	...
	aGc.DrawArc(smile, TPoint(10, circleSize - shrinkSize), TPoint(circleSize,
...
void CFbsControl::CreateRotatedBitmapL()
	...
	iBitmap4->Create(TSize(inSize.iHeight, inSize.iWidth),
			iBitmap1->DisplayMode());
...
void CFbsControl::DrawTwoBitmaps(CFbsBitmap* aBitmap1, CFbsBitmap* aBitmap2,
		CWindowGc& aGc) const
	{
	// calculate position for top left of bitmap so it is centered
